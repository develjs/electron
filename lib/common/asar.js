(function () {
  ...
  exports.wrapFsWithAsar = function (fs) {
    ...
    fs.readFile = function (p, options, callback) {
      ...
      const _buffer = new ArrayBuffer(info.size) // buffer must be based on ArrayBuffer for transfer to C code
      const buffer = new Buffer(_buffer)
      ...
      fs.read(fd, buffer, 0, info.size, info.offset, function (error) {
        if (archive.encodeBuffer) archive.encodeBuffer(_buffer);
    ...
    fs.readFileSync = function (p, options) {
      ...
      const {encoding} = options
      const _buffer = new ArrayBuffer(info.size)
      const buffer = new Buffer(_buffer)
      const fd = archive.getFd()
      if (!(fd >= 0)) {
        notFoundError(asarPath, filePath)
